...
function createCinema() {
  ...
  for (var i = 0; i < 6; i++) {
    cinema[i][7].isSeat = false;
    for (var j = 0; j < 10; j++) {
      cinema[3][j].isSeat = false;
    }
  }
...
function renderCinema() {
  ...
  for (var i = 0; i < gCinema.length; i++) {
    ...
    for (var j = 0; j < gCinema[0].length; j++) {
      ...
      strHTML += `\t<td title="${title}" class="cell ${className} cell-${i}-${j}" 
...
function renderCell(location, value = 'show') {
...
function showAvailableSeats() {
  console.log('gAvailableSeats', gAvailableSeats);

  for (let i = 0; i < gAvailableSeats.length; i++) {
    const seat = gAvailableSeats[i];
    renderCell(seat, 'show');

    setTimeout(renderCell, 2500, seat);
  }
}
